...
@protocol SPSimplePingDelegate;

/*! Controls the IP address version used by SimplePing instances.
 */

typedef NS_ENUM(NSInteger, SPSimplePingAddressStyle) {
    SPSimplePingAddressStyleAny,          ///< Use the first IPv4 or IPv6 address found; the default.
    SPSimplePingAddressStyleICMPv4,       ///< Use the first IPv4 address found.
    SPSimplePingAddressStyleICMPv6        ///< Use the first IPv6 address found.
};
...
@interface SPSimplePing : NSObject
...
@property (nonatomic, weak, readwrite, nullable) id<SPSimplePingDelegate> delegate;

/*! Controls the IP address version used by the object.
 *  \details You should set this value before starting the object.
 */

@property (nonatomic, assign, readwrite) SPSimplePingAddressStyle addressStyle;
...
@protocol SPSimplePingDelegate <NSObject>
...
typedef struct SPICMPHeader SPICMPHeader;

__Check_Compile_Time(sizeof(SPICMPHeader) == 8);
__Check_Compile_Time(offsetof(SPICMPHeader, type) == 0);
__Check_Compile_Time(offsetof(SPICMPHeader, code) == 1);
__Check_Compile_Time(offsetof(SPICMPHeader, checksum) == 2);
__Check_Compile_Time(offsetof(SPICMPHeader, identifier) == 4);
__Check_Compile_Time(offsetof(SPICMPHeader, sequenceNumber) == 6);

enum {
    SPICMPv4TypeEchoRequest = 8,          ///< The ICMP `type` for a ping request; in this case `code` is always 0.
    SPICMPv4TypeEchoReply   = 0           ///< The ICMP `type` for a ping response; in this case `code` is always 0.
};

enum {
    SPICMPv6TypeEchoRequest = 128,        ///< The ICMP `type` for a ping request; in this case `code` is always 0.
    SPICMPv6TypeEchoReply   = 129         ///< The ICMP `type` for a ping response; in this case `code` is always 0.
};
